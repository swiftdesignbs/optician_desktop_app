...
class _ProductMasterState extends State<ProductMaster> {
  ...
  Container ProductSectionPartThree() {
    return Container(
      color: Colors.white,
      child: Column(
        children: [
...
// Inside your onPressed:
                  onPressed: () async {
                    try {
                      final product = ProductMasterCompanion(
                        productCode: drift.Value(productCode.text),
                        productName: drift.Value(productName.text),
                        category: drift.Value(selectedCategory ?? ''),
                        material: drift.Value(selectedMaterial ?? ''),
                        brand: drift.Value(selectedBrand ?? ''),
                        subBrand: drift.Value(selectedSubBrand ?? ''),
                        size: drift.Value(selectedSize ?? ''),
                        colors: drift.Value(selectedColor ?? ''),
                        lensType1: drift.Value(selectedLens1 ?? ''),
                        lensType2: drift.Value(selectedLens2 ?? ''),
                        shape: drift.Value(selectedShape ?? ''),
                        gender: drift.Value(selectedGender ?? ''),
                        allowNegativeStock:
                            drift.Value(selectedNegativeStock == 'Yes'),
                        ...
                        createdDate: drift.Value(pickedDate ?? DateTime.now()),
                      );
  ...
  Container ProductSectionPartTwo() {
    return Container(
      color: Colors.white,
      child: Column(
        children: [
          ...
          CustomDropdown(
            ...
            onChanged: (value) => setState(() => selectedNegativeStock = value),
          ),
          SizedBox(height: 10),
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
  ...
  Container ProductSectionPartOne() {
    return Container(
      color: Colors.white,
      child: Column(
        children: [
            ...
            children: [
              ...
              Expanded(
                child: CustomDropdown(
                  ...
                  onChanged: (value) =>
                      setState(() => selectedSubBrand = value),
                ),
              ),
            ],
          ),
            ...
            children: [
              Expanded(
                child: CustomDropdown(
                  ...
                  items: lensTypes1,
                  value: selectedLens1,
                  onChanged: (value) => setState(() => selectedLens1 = value),
                ),
              ),
              SizedBox(
                width: 10,
              ...
              Expanded(
                child: CustomDropdown(
                  ...
                  items: lensTypes2,
                  value: selectedLens2,
                  onChanged: (value) => setState(() => selectedLens2 = value),
                ),
              ),
            ],
          ),
